...
expenseForm.addEventListener('submit',(e)=>{
  ...
  axios.post('http://54.235.24.107:3000/expense/add-expenses',details,{headers:{"Authorization":token}}).then((res)=>{
      expenseForm.reset()
...
function getAllProducts(page,limit){
  const token=localStorage.getItem('token')
  tbody.innerHTML=""
  axios.get(`http://54.235.24.107:3000/expense/get-expenses?page=${page}&expensePerPage=${limit}`,{headers:{"Authorization":token}}).then((res)=>{
...
function deleteExpense(id,page,e) {
  const token = localStorage.getItem('token');
  axios.delete(`http://54.235.24.107:3000/expense/delete-expense/${id}`, { headers: { "Authorization": token } })
      .then((res) => {
...
premiumButton.addEventListener('click',(e)=>{
  ...
  axios.get('http://54.235.24.107:3000/purchase/premiummembership',{headers:{"Authorization":token}}).then((res)=>{
      // console.log(res)
      const options = {
          "key": res.data.key_id,
          ...
          "handler": async function (response) {
              try {
                 ...
                 const res= await axios.post('http://54.235.24.107:3000/purchase/updatetransactionstatus', {
...
download.addEventListener('click',()=>{
  const token=localStorage.getItem('token')
      axios.get("http://54.235.24.107:3000/expense/downloadExpenses",{headers:{"Authorization":token}}).then((res)=>{
