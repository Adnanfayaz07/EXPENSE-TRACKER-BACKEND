...
dayExpenseShowBtn.addEventListener('click', () => {
  ...
  const day=document.querySelector('#day')
    ...
    axios.get(`http://44.206.201.3:3000/expense/day-expenses?day=${Day}&month=${Month}&year=${Year}`,{ headers: { "Authorization": token } })
      .then((res) => {
  ...
  monthshowbtn.addEventListener('click',()=>{
    ...
    const token = localStorage.getItem('token');
    tbody.innerHTML=''
    axios.get(`http://44.206.201.3:3000/expense/month-expenses?month=${Month}&year=${Year}`,{ headers: { "Authorization": token } })
      .then((res) => {
